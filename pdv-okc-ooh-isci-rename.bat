...
rem  Statics are .tif, digitals are .png. Every pattern requires OKC + the exact size,
rem  so Tulsa files in the same folder are never touched, and the Jr. Posters are
rem  matched before the full-size posters so the two never collide. The digital
rem  patterns also match already-renamed files, so it is safe to re-run over an
rem  earlier pass that used the DB003/DB004 codes.
...
for %%F in ("*OKC*Thunder*400x1400*.png") do if not exist "OKCPDV26DB001O - PDV Digital Bulletin - Oklahoma City - Thunder - 400x1400.png" (ren "%%F" "OKCPDV26DB001O - PDV Digital Bulletin - Oklahoma City - Thunder - 400x1400.png" & echo   renamed  %%F  to  OKCPDV26DB001O)
for %%F in ("*OKC*Pepper*400x1400*.png") do if not exist "OKCPDV26DB002O - PDV Digital Bulletin - Oklahoma City - Pepper and Murry - 400x1400.png" (ren "%%F" "OKCPDV26DB002O - PDV Digital Bulletin - Oklahoma City - Pepper and Murry - 400x1400.png" & echo   renamed  %%F  to  OKCPDV26DB002O)
for %%F in ("*OKC*Thunder*400x840*.png") do if not exist "OKCPDV26DP001O - PDV Digital Poster - Oklahoma City - Thunder - 400x840.png" (ren "%%F" "OKCPDV26DP001O - PDV Digital Poster - Oklahoma City - Thunder - 400x840.png" & echo   renamed  %%F  to  OKCPDV26DP001O)
for %%F in ("*OKC*Pepper*400x840*.png") do if not exist "OKCPDV26DP002O - PDV Digital Poster - Oklahoma City - Pepper and Murry - 400x840.png" (ren "%%F" "OKCPDV26DP002O - PDV Digital Poster - Oklahoma City - Pepper and Murry - 400x840.png" & echo   renamed  %%F  to  OKCPDV26DP002O)
echo.
