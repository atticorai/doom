...
const App=()=>{
  ...
  const OohPg=()=>{
    ...
    // June 2026 creative tagging — temp ISCIs derived from JPG filenames
    // so the user can see clumping on the map before producing real
    // creative codes. Maps Lamar boardId → temp ISCI string.
    ...
    // Creative theme colors for the heat map — short labels for legend
    ...
    const applyJune26=()=>{
      if(!confirm("Apply June 2026 temp creative tags to "+Object.keys(WK_JUNE26_CREATIVE).length+" boards? This sets a temp ISCI on each so they show on the heat map color-coded by creative."))return;
    ...
    return<div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      {viewMode==="cards"?<CardGrid/>:
       viewMode==="map"?<Cd><div style={{padding:10}}>
        ...
        {(()=>{const colorByCreative=oohMapColorBy==="creative";const fallback=(p)=>dmaColors[p.market]||"#D4A040";const colorFn=colorByCreative?(p=>{const isci=p.isci||"";if(creativeColors[isci])return creativeColors[isci].color;if(!isci)return"#4a3565";let h=0;for(let i=0;i<isci.length;i++)h=(h*31+isci.charCodeAt(i))&0xfff;const palette=["#D4A040","#9b7bb0","#4AC8E8","#5BC4A0","#E85A7A","#C4A0C8","#059669","#ec4899"];return palette[h%palette.length]}):fallback;return<><OohMap pins={fl.map(p=>{const co=WK_COORDS[p.boardId];return{id:p.boardId,lat:co?co[0]:0,lng:co?co[1]:0,location:p.location,vendor:p.vendor,size:p.size,status:p.isci?"ISCI: "+p.isci:"Active",impressions:p.impressions,market:p.dma,isci:p.isci,closeImg:p.closeImg}})} colorFn={colorFn} height={420}/>
